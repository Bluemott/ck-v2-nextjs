...
export default function IndexNowSubmitter() {
  ...
  const [config, setConfig] = useState<IndexNowConfig | null>(null);
  ...
  useEffect(() => {
    const loadConfig = async () => {
      try {
        const configData = await getIndexNowConfig();
        setConfig(configData);
      } catch (error) {
        console.error('Failed to load IndexNow config:', error);
        setConfig({
          isConfigured: false,
          key: 'Error loading config',
          host: 'Unknown',
          keyLocation: null,
          endpoints: []
        });
    ...
    return (
      <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
        <div className="flex items-center justify-center py-8">
          ...
          <span className="ml-3 text-gray-600">Loading IndexNow configuration...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      <div className="mb-6 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          ...
          {config?.keyFileAccessible !== undefined && (
            <div>
              <span className="font-medium">Key File Accessible: </span>
              <span className={config.keyFileAccessible ? 'text-green-600' : 'text-red-600'}>
                {config.keyFileAccessible ? 'Yes' : 'No'}
              </span>
            </div>
          )}
        </div>
