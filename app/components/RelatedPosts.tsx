...
import { fetchRelatedPosts, type WPGraphQLPost, decodeHtmlEntities } from '../lib/wpgraphql';
...
const RelatedPosts = ({ currentPost, categories, tags, limit = 3 }: RelatedPostsProps) => {
  ...
  if (error) {
    return (
      <div className="bg-white rounded-xl shadow-lg p-6 lg:p-8 border border-gray-100 text-center">
        ...
        <h3 className="text-lg font-semibold mb-2 text-red-600">Error Loading Related Posts</h3>
  ...
  if (relatedPosts.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-lg p-6 lg:p-8 border border-gray-100 text-center">
        ...
        <h3 className="text-lg font-semibold mb-2 text-gray-600">No Related Posts</h3>
  ...
  const hasFallbackContent = relatedPosts.some(post => 
    post._relatedReasons?.includes('Recent post (fallback)') || 
    post._relatedReasons?.includes('Fallback content')
  );

  const hasHighQualityMatches = relatedPosts.some(post => 
    (post._relatedScore || 0) > 5
  );

  return (
    <div className="bg-white rounded-xl shadow-lg p-6 lg:p-8 border border-gray-100">
      <h3 className="text-xl font-bold mb-6 text-gray-800">
        {hasHighQualityMatches ? 'Related Posts' : 'You Might Also Like'}
      </h3>
      {hasFallbackContent && !hasHighQualityMatches && (
        <p className="text-sm text-gray-500 mb-6 italic">
          Showing recent posts from our blog
        </p>
      )}
      <div className="space-y-6">
