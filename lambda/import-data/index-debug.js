...
// Import posts with detailed error handling
async function importPosts(client, posts) {
  ...
  for (let i = 0; i < posts.length; i++) {
    const post = posts[i];
    console.log(`📝 Processing post ${i + 1}/${posts.length}: ${post.title?.rendered || post.id}`);
    ...
    try {
      ...
      console.log(`   Values for post ${post.id}:`, {
        author: values[0],
        date: values[1],
        title: values[4],
        content_length: values[3]?.length || 0,
        wordpress_id: values[23]
      });
      
      await client.query(insertSQL, values);
      successCount++;
      console.log(`   ✅ Post ${post.id} imported successfully`);
  ...
  console.log(`✅ Posts import completed: ${successCount} successful, ${errorCount} failed`);
  return { successCount, errorCount };
}

// Import categories with detailed error handling
async function importCategories(client, categories) {
  ...
  for (let i = 0; i < categories.length; i++) {
    ...
    try {
      ...
      successCount++;
      console.log(`   ✅ Category ${category.id} imported successfully`);
  ...
  console.log(`✅ Categories import completed: ${successCount} successful, ${errorCount} failed`);
  return { successCount, errorCount };
}

// Import tags with detailed error handling
async function importTags(client, tags) {
  ...
  for (let i = 0; i < tags.length; i++) {
    const tag = tags[i];
    console.log(`🏷️  Processing tag ${i + 1}/${tags.length}: ${tag.name}`);
    ...
    try {
      ...
      successCount++;
      console.log(`   ✅ Tag ${tag.id} imported successfully`);
  ...
  console.log(`✅ Tags import completed: ${successCount} successful, ${errorCount} failed`);
  return { successCount, errorCount };
}
...
exports.handler = async (event) => {
  try {
    ...
    // Import data with detailed tracking
    let importResults = {
      posts: { successCount: 0, errorCount: 0 },
      categories: { successCount: 0, errorCount: 0 },
      tags: { successCount: 0, errorCount: 0 }
    };
    
    if (posts.length > 0) {
      importResults.posts = await importPosts(client, posts);
    }
    
    if (categories.length > 0) {
      importResults.categories = await importCategories(client, categories);
    }
    
    if (tags.length > 0) {
      importResults.tags = await importTags(client, tags);
    }
