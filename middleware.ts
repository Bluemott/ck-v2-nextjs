...
export async function middleware(request: NextRequest) {
  ...
  // Debug logging for tag/category redirects
  if (
    pathname.startsWith('/blog/tag/') ||
    pathname.startsWith('/blog/category/')
  ) {
    console.warn('[Middleware] Tag/Category URL detected:', {
      pathname,
      url: request.url,
  ...
  try {
    ...
    if (
      pathname.match(
        /\.(css|js|png|jpg|jpeg|gif|ico|svg|webp|avif|woff|woff2|ttf|eot)$/
      )
    ) {
      response.headers.set(
        'Cache-Control',
